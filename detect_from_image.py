...
threshold=0.1
output_directory = 'outputs/hasilbb'
...
def make_detection_csv(output):
    file_name = "sampah/output_resnet101_momentum.csv"
    ...
    with open(file_name, mode='w', newline='') as file:
        ...
        writer.writerow(["filename", "label", "score", "xmin", "ymin", "xmax", "ymax"])

        # Write the list elements as rows in the CSV file
        for item in output:
        ...
        for i_path in image_paths:
            ...
            for index, score in enumerate(output_dict['detection_scores']):
                if score > threshold:
                    ...
                    output.append([file_name, label, score, int(xmin * image_width), int(ymin * image_height), int(xmax * image_width), int(ymax * image_height)])
